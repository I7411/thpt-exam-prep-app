                          ...
                          if (formKey.currentState!.validate()) {
                            ...
                            try {
                              ...
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text('Tạo tài khoản thất bại: $e'),
                                  ),
                                );
                              }
                            }
                      ...
                      : () async {
                          if (formKey.currentState!.validate()) {
                            ...
                            try {
                              ...
                              await context.read<AdminController>().updateUser(
                                updatedUser,
                              );
                              if (dialogContext.mounted) {
                                Navigator.of(dialogContext).pop();
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            return AlertDialog(
              ...
              content: Text(
                'Bạn có chắc chắn muốn xóa tài khoản "${user.fullName}" (${user.email}) không? Hành động này không thể hoàn tác.',
              ),
              actions: [
                TextButton(
                  onPressed: localLoading
                ...
                ElevatedButton(
                  onPressed: localLoading
                      ...
                      : () async {
                          ...
                          try {
                            await context.read<AdminController>().deleteUser(
                              user.id,
                            );
                            if (dialogContext.mounted) {
                              Navigator.of(dialogContext).pop();
